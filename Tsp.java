...
public class Tsp {
  ...
  public static long[][] dm = null;
  Tsp(long[][] dm){
    // System.out.print(dm);
    this.dm =  dm;
  }
  static class DataModel {
    public final long[][] distanceMatrix = dm;
    // {
//         {0,20,42,25},
//         {20,0,30,34},
//         {42,30,0,10},
...
// };
  ...
  static long[][] parseArray(String s){
    s=s.replace("[","");//replacing all [ to ""
    s=s.substring(0,s.length()-2);//ignoring last two ]]
    String s1[]=s.split("],");//separating all by "],"

    long my_matrics[][] = new long[s1.length][s1.length];//declaring two dimensional matrix for input
    ...
    for(int i=0;i<s1.length;i++){
        ...
        for(int j=0;j<single_int.length;j++){
            my_matrics[i][j]=Long.parseLong(single_int[j]);//adding single values
        }
    }
  ...
  public static void main(String[] args) throws Exception {
    ...
    // String x = args[0];
    long[][] dsm = parseArray(args[0]);
  //  System.out.print("LEEEEEEENTTTTTTHHHHHHH"+x+"\n");
	...
	//logger.info(data.distanceMatrix.length);
